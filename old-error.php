<?php
    /*
     Plugin Name: Cloudflare Rocketscript
     Plugin URI: http://nabtron.com/cloudflare-rocketscript
     Donate link: payment@nabtron.com
     Description: disables or enables cloudflare rocket script on specific files
     Author: nabtron
     Author URI: http://nabtron.com/
     Version: 0.1
     Min WP Version: 4.4
    Max WP Version: 4.4
     */
// prevent direct access
defined( 'ABSPATH' ) or die( 'Plugin file cannot be accessed directly.' );

// Update routines
if ('insert' == $_POST['action_nabcfrs']) {
        update_option("nabcfrs_true",$_POST['nabcfrs_true']);
        update_option("nabcfrs_false",$_POST['nabcfrs_false']);
}

if (!class_exists('nabcfrs_main')) {
	class nabcfrs_main {
		// PHP 5 Constructor
		function __construct(){
			add_action('admin_menu', 'nabcfrs_settings');
            add_filter('script_loader_tag', 'nabcfrs_truefalse', 10, 2);
		}
	}

	function nabcfrs_settings() {
	?>
		<!-- Start Options Admin area -->
		<div class="wrap">
		  <h2>Cloudflare Rocketscript Options</h2>
		  <div style="margin-top:20px;">
		    <form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>&amp;updated=true">
		      <div style="">
		      <table class="form-table">
		        <tr>
		          <th scope="col" colspan="3" cellpadding="15"><strong>Settings</strong></th>
		        </tr>
		        <tr>
		          <th scope="row"><strong>True</strong></th>
		          <td><input name="nabcfrs_true" size="10" value="<?=get_option("nabcfrs_true");?>" type="text" />
		            <br />comma seperated JS filenames to set data-cfasync="false" for</td>
		        </tr>
		        <tr>
		          <th scope="row"><strong>False</strong></th>
		          <td><input name="nabcfrs_false" size="10" value="<?=get_option("nabcfrs_false");?>" type="text" />
		            <br />comma seperated JS filenames to set data-cfasync="false" for</td>
		        </tr>
		      </table>
		      <br>
		      <p class="nabcfrs_submit">
		        <input name="nabcfrs_submit" type="submit" id="nabcfrs_submit" value="Save changes &raquo;">
		      </p>
		    </form>
		    <br /><br /><hr />
		    <center><h4>Developed by <a href="http://nabtron.com/" target="_blank">Nabtron</a>.</h4></center>
		  </div>
		</div>

	<?php
	} // End function nabcfrs_settings()

	// Admin menu Option
	function nabwrap_description_add_menu() {
		add_options_page('Cloudflare Rocketscript', 'Cloudflare Rocketscript', 'manage_options', __FILE__, 'nabcfrs_settings');
	}

	function get_nabwrapper_id($content) {
		if(!is_admin()) { 
			nabcfrs_true  = get_option('nabcfrs_true');
			nabcfrs_false  = get_option('nabcfrs_false');
		}
	}

    function nabcfrs_truefalse($tag) {
        
        // get list of files for both true and false
        $nabcfrs_true  = implode(get_option('nabcfrs_true'), ',');
        $nabcfrs_false  = implode(get_option('nabcfrs_false', ',');
        
        foreach($nabcfrs_true as $true_script){
            if(true == strpos($tag, $true_script ) )
                return str_replace( ' src', ' data-cfasync="true" src', $tag );
        }

        foreach($nabcfrs_false as $false_script){
            if(true == strpos($tag, $false_script ) )
                return str_replace( ' src', ' data-cfasync="false" src', $tag );
        }
    }
}

//instantiate the class
if (class_exists('nabcfrs_main')) {
	$nabwrap_main = new nabcfrs_main();
}
            

/*
add_filter('script_loader_tag', 'nab_remove_cfasync', 10, 2);
    function nab_remove_cfasync($tag) {
        
        // array of scripts to add data-cfasync="false"
        $scripts_to_defer = array('jquery.min.js', 'jquery.js', 'jquery-migrate.min.js', 'greensock.js', 'layerslider.kreaturamedia.jquery.js', 'layerslider.transitions.js', 'jquery.themepunch.revolution.min.js', 'eea_gmap.js');
        
        foreach($scripts_to_defer as $defer_script){
            if(true == strpos($tag, $defer_script ) )
                return str_replace( ' src', ' data-cfasync="false" src', $tag );
        }

/*
        // array of scripts to add data-cfasync="true"
        $scripts_to_defer = array('one.js, two.js');
        
        foreach($scripts_to_defer as $defer_script){
            if(true == strpos($tag, $defer_script ) )
                return str_replace( ' src', ' data-cfasync="true" src', $tag );
        }


        
        return $tag;
    }
*/    
?>
